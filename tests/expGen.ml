      ...
      (fun self params ->
        if params.size = 0 then atom ~params
          ...
          let manyop =
            ...
            in
            return @@ ExpTyped.manyop op exps
          in
          let ite =
            ...
            return @@ ExpTyped.ite ~cond expt expf
          in
          let+ exp = oneof [atom; unop; binop; manyop; ite] in
          assert (ExpTyped.get_type exp = params.typ);
          exp)
      params

  let bv_consts size =
    assert (size > 0);
    bitvec_size size >|= ExpTyped.bits

  let bv_atom_from_var (var_gen : 'v sized) size =
    let var = var_gen size >|= ExpTyped.var ~typ:(Ty_BitVec size) in
    oneof [var; bv_consts size]

  let bv_atom_with_var = bv_atom_from_var bv_var

  let bool_consts = bool >|= ExpTyped.bool

  let bool_atom_from_var (var_gen : 'v t) =
    let var = var_gen >|= ExpTyped.var ~typ:Ty_Bool in
...
let rec shrink_propagate (shrinker : ('v, 'm) ExpTyped.t Q.Shrink.t) exp yield =
