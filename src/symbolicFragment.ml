...
module Make (Var : Exp.Var) : S with type var = Var.t = struct
  ...
  module Block = struct
    ...
    let map_exp (f : Exp.t -> Exp.t) (b : t) : t = { b with base = Opt.map f b.base }

    let iter_exp (f : Exp.t -> unit) (b : t) : unit = Opt.iter f b.base
  ...
  let write frag (block : Block.t) exp =
    ...
    | Some bbase ->
        ...
        let new_block =
          ...
          | None ->
              let (start, endp) =
                Opt.value block.bounds ~default:(Int.min_int / 2, (Int.max_int - 1) / 2)
              in
