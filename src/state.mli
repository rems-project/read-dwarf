...
val copy : ?elf:ElfFile.t -> t -> t

(** Copy the state with {!copy} if and only if it is locked.
    The returned state is always unlocked *)
val copy_if_locked : ?elf:ElfFile.t -> t -> t
...
val make_read : t -> ?ctyp:Ctype.t -> AstDef.Size.t -> var

(** Set a {!Var.ReadVar} to a specific value in {!t.read_vars} *)
val set_read : t -> int -> exp -> unit

(** Read memory from rodata *)
val read_from_rodata : t -> addr:exp -> size:AstDef.Size.t -> exp option
...
val read :
  provenance:Ctype.provenance -> ?ctyp:Ctype.t -> t -> addr:exp -> size:AstDef.Size.t -> exp
...
val read_noprov : ?ctyp:Ctype.t -> t -> addr:exp -> size:AstDef.Size.t -> exp

(** Write the provided value in the block. Mutate the state.*)
val write : provenance:Ctype.provenance -> t -> addr:exp -> size:AstDef.Size.t -> exp -> unit
...
val write_noprov : t -> addr:exp -> size:AstDef.Size.t -> exp -> unit
