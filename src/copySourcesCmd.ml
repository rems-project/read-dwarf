...
let src_target_dir =
  let doc = "Path to target directory to copy sources into (e.g. to use later as comp_dir argument)" in
...
let info =
  let doc =
    "Copies the source files mentioned in the DWARF information of an ELF file, to the src_target_dir directory.  This can then be used, for example, as the argument to comp_dir, to run read-dwarf without the original build tree."
  in
